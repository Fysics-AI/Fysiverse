...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const videoMap = {
        ...
        'balloon': 'vid/sec2/monkey_drop_website.mp4',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const compItems = [
        {
            name: 'open_laptop',
            text: '',
            caption: "文本: 用左臂稳稳地抬起那台屏幕平整的翻盖式笔记本电脑。"
        },
        {
            name: 'place_bread_skillet',
            text: '',
            caption: "文本: 拿起面包，把它丢进圆形煎锅里。"
        },
    ];
    ...
    compItems.forEach((item, index) => {
        ...
        btn.addEventListener('click', function () {
            ...
            const src = `vid/sec5/${demoName}_output_website.mp4`;
    ...
    const compItems = [
        {
            ...
            caption: "文本: 地上放着四个柜子，最左边的那个向右倒下，结果把其他的柜子也带倒了。"
        },
        {
            name: 'bear-bounce',
            file: 'bear_bounce_website.mp4',
            caption: "文本: 一只泰迪熊向右上方弹起，随后在重力作用下自然下落，整个过程流畅且逼真。"
        },
        {
            name: 'bunny-fluid',
            file: 'bunny_fluid_website.mp4',
            caption: "文本: 一簇兔子形状的水团在重力作用下坠落并飞溅开来，整个过程流畅自然。"
        },
    ];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elasticVideoPlayer = document.getElementById('elastic-video-player');
    const elasticValueDisplay = document.getElementById('elastic-value');

    if (!elasticSlider || !elasticVideoPlayer) return;

    elasticValueDisplay.textContent = "Elastic Level-" + elasticSlider.value;

    elasticSlider.addEventListener('input', () => {
        const level = elasticSlider.value;
        elasticValueDisplay.textContent = "Elastic Level-" + level;

        const newSrc = `vid/sec12/k_${level}_website.mp4`;
        elasticVideoPlayer.src = newSrc;
        elasticVideoPlayer.load();

        elasticVideoPlayer.play().catch(e => {
            console.warn("Auto-play prevented:", e);
        });
    });
});